...
// Initialize Resend safely (returns null if key is missing)
...
export async function sendEmail(
  ...
  try {
    ...
    }/api/track/open/${leadId}" width="1" height="1" style="display:none" />`;
    ...
    // 4. Send email via Resend OR Log it (Mock Mode)
    let messageId = null;

    if (!resend) {
      console.log(
        "⚠️ [MOCK EMAIL] No RESEND_API_KEY found. Logging email instead."
      );
      console.log("--- EMAIL START ---");
      console.log(`To: ${to}`);
      console.log(`Subject: ${subject}`);
      console.log(`Text Body: ${finalTextBody.substring(0, 100)}...`);
    ...
    try {
      await insert<Partial<EmailLog>>("email_logs", {
        ...
        sent_at: new Date().toISOString(), // Ensure sent_at is recorded
      });
    } catch (logError) {
    ...
    try {
      await update("leads", leadId, {
        ...
        status: "emailed", // Auto-update status
      });
    } catch (updateError) {
...
function wrapLinksWithTracking(html: string, leadId: string): string {
  ...
  return html.replace(linkRegex, (match, attributes, originalUrl) => {
    ...
    // Create tracking URL
    const trackingUrl = `${baseUrl}/api/track/click/${leadId}?url=${encodeURIComponent(
      originalUrl
    )}`;
